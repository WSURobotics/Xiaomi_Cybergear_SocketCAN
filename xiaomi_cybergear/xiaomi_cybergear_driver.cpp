...
int XiaomiCyberGearDriver::init_can(const std::string& can_interface, bool serial_debug) {
    _use_serial_debug = serial_debug;
    interface_name = can_interface;

    can_socket = socket(PF_CAN, SOCK_RAW, CAN_RAW);
    if (can_socket < 0) {
        if (_use_serial_debug) std::cerr << "Error opening CAN socket" << std::endl;
        return -1;
    }
    ...
    if (ioctl(can_socket, SIOCGIFINDEX, &ifr) < 0) {
        if (_use_serial_debug) std::cerr << "Error getting interface index" << std::endl;
        return -1;
    }
    ...
    if (bind(can_socket, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        if (_use_serial_debug) std::cerr << "Error binding CAN socket" << std::endl;
        return -1;
    }

    if (_use_serial_debug) std::cout << "CAN driver initialized and bound to " << can_interface << std::endl;
    return 0;
}
...
// }
void XiaomiCyberGearDriver::process_message(struct can_frame& frame) {
    uint16_t raw_position = frame.data[1] | (frame.data[0] << 8);
    uint16_t raw_speed = frame.data[3] | (frame.data[2] << 8);
    uint16_t raw_torque = frame.data[5] | (frame.data[4] << 8);
    uint16_t raw_temperature = frame.data[7] | (frame.data[6] << 8);
